...
class ExamplesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create example" do
    assert_difference('Example.count') do
      post examples_url, params: { example: { example: @example.example, word_id: @example.word_id } }
    end
  ...
  test "should update example" do
    patch example_url(@example), params: { example: { example: @example.example, word_id: @example.word_id } }
